...
func TestPack_Unpack(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("recover...:", r)
		}
	...
	endi := KOS.GetEndian()
	//data := []int{-59, -40, 0, 3, 24}
	bin, err := KConv.Pack(endi, t1)
	println(1111, bin)
	if err != nil {
		println(err.Error())
		t.Error("Pack fail")
	...
	sour, err := KConv.Unpack(endi, bin)
	fmt.Printf("2222: %v\n", sour)
	if err != nil {
		t.Error("Unpack fail")
		return
	...
	//data不能是字符串
	_, err = KConv.Pack(endi, "hello world!")
	if err == nil {
		t.Error("Pack fail")
		return
	}
	...
	//data可以是字节切片
	_, err = KConv.Pack(endi, []byte("hello world!"))
	if err == nil {
		t.Error("Pack fail")
		return
	}
