...
func TestEncrypt_AuthCode(t *testing.T) {
	...
	res, exp, err = KEncr.AuthCode(bytsHello, bytSpeedLight, true, 0)
	assert.NotNil(t, res)
	assert.Nil(t, err)

	res2, exp, err = KEncr.AuthCode(res, bytSpeedLight, false, 0)
	assert.Equal(t, string(bytsHello), string(res2))
	assert.Nil(t, err)

	//过期
	res, exp, err = KEncr.AuthCode(bytsHello, bytSpeedLight, true, -3600)
	res2, exp, err = KEncr.AuthCode(res, bytSpeedLight, false, 0)
	assert.Empty(t, res2)
	...
	//空串
	res, exp, err = KEncr.AuthCode(bytEmpty, bytSpeedLight, true, 0)
	assert.Empty(t, res)
	assert.NotNil(t, err)

	//空密钥
	res, exp, err = KEncr.AuthCode(bytsHello, bytEmpty, true, 0)
	assert.NotEmpty(t, res)

	//解密校验
	res2, exp, err = KEncr.AuthCode([]byte(strSha512), bytSpeedLight, false, 0)
	assert.Empty(t, res2)
	...
	res2, exp, err = KEncr.AuthCode(str1, []byte(strEmptyMd5), false, 0)
	assert.Equal(t, string(res2), strHello2)
	assert.Nil(t, err)

	res2, exp, err = KEncr.AuthCode(str2, []byte(strEmptyMd5), false, 0)
	assert.Equal(t, string(res2), strHelloEmoji)
	assert.Nil(t, err)

	res2, exp, err = KEncr.AuthCode(str3, []byte(strEmptyMd5[0:16]), false, 0)
...
func TestEncrypt_RsaPublicEncryptPrivateDecrypt(t *testing.T) {
	...
	//PKCS8的私钥
	enc, err = KEncr.RsaPublicEncrypt(bytsHello, pubFileBs2048PKCS8)
	assert.Nil(t, err)
	des, err = KEncr.RsaPrivateDecrypt(enc, priFileBs2048PKCS8)
	assert.Nil(t, err)
	assert.Equal(t, string(bytsHello), string(des))
...
func TestEncrypt_RsaPrivateEncryptPublicDecrypt(t *testing.T) {
	...
	//PKCS8的私钥
	enc, err = KEncr.RsaPrivateEncrypt(bytsHello, priFileBs2048PKCS8)
	assert.Nil(t, err)
	des, err = KEncr.RsaPublicDecrypt(enc, pubFileBs2048PKCS8)
	assert.Nil(t, err)
	assert.Equal(t, string(bytsHello), string(des))
...
func TestEncrypt_RsaPublicEncryptPrivateDecrypt_Long(t *testing.T) {
	...
	//PKCS8的私钥
	enc, err = KEncr.RsaPublicEncryptLong(cont, pubFileBs2048PKCS8)
	assert.Nil(t, err)
	des, err = KEncr.RsaPrivateDecryptLong(enc, priFileBs2048PKCS8)
	assert.Nil(t, err)
	assert.Equal(t, tesHtmlDoc, string(des))
