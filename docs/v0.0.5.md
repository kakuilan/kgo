...
```
RsaPrivateDecrypt RSA私钥解密. cipherText为密文,privateKey为私钥.
...
```
RsaPrivateEncrypt RSA私钥加密. clearText为明文,privateKey为私钥.
...
```
RsaPublicDecrypt RSA公钥解密.比加密耗时. cipherText为密文,publicKey为公钥.
...
```
RsaPublicEncrypt RSA公钥加密.比解密耗时. clearText为明文,publicKey为公钥.
...
```
MemoryUsage 获取内存使用率(仅支持linux),单位字节. 参数 virtual,是否取虚拟内存. used为已用, free为空闲,
total为总数.
...
```
ParseUrl 解析 URL,返回其组成部分. component为需要返回的组成; -1: all; 1: scheme; 2: host; 4:
port; 8: user; 16: pass; 32: path; 64: query; 128: fragment .
...
```
VersionCompare 对比两个版本号字符串. 在第一个版本低于第二个时,version_compare() 返回 -1;如果两者相等,返回
0;第二个版本更低时则返回 1. operator允许的操作符有: <, lt, <=, le, >, gt, >=, ge, ==, =, eq, !=,
<>, ne . 特定的版本字符串，将会用以下顺序处理： 未找到的任意字符串 < dev < alpha = a < beta = b < RC = rc <
# < pl = p 用法: VersionCompare("1.2.3-alpha", "1.2.3RC7", '>=') ;
